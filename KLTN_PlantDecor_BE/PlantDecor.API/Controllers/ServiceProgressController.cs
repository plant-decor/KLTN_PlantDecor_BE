...
namespace PlantDecor.API.Controllers
{
    ...
    public class ServiceProgressController : ControllerBase
    {
        ...
        /// <summary>
        /// [Customer] Gửi nhận xét cho một buổi chăm sóc đã hoàn thành
        /// </summary>
        [HttpPost("{id}/customer-comment")]
        [Authorize(Roles = "Customer")]
        public async Task<IActionResult> SubmitCustomerComment(int id, [FromBody] CreateServiceProgressCommentRequestDto request)
        {
            var customerId = GetUserId();
            var result = await _serviceProgressService.SubmitCustomerCommentAsync(customerId, id, request);
            return Ok(new ApiResponse<ServiceProgressResponseDto>
            {
                Success = true,
                StatusCode = StatusCodes.Status200OK,
        ...
        [HttpGet("my-schedule")]
        [Authorize(Roles = "Caretaker")]
        public async Task<IActionResult> GetMySchedule([FromQuery] DateOnly? from, [FromQuery] DateOnly? to)
        {
            var caretakerId = GetUserId();
            var fromDate = from ?? DateOnly.FromDateTime(DateTime.Today);
            var toDate = to ?? fromDate.AddDays(6);
            var result = await _serviceProgressService.GetMyScheduleAsync(caretakerId, fromDate, toDate);
            return Ok(new ApiResponse<List<ServiceProgressResponseDto>>
            {
                Success = true,
                StatusCode = StatusCodes.Status200OK,
                Message = "Get my schedule successfully",
                Payload = result
            });
        }
