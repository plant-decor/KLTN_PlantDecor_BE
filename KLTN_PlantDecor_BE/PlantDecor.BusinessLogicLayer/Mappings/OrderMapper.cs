...
namespace PlantDecor.BusinessLogicLayer.Mappings
{
    public static class OrderMapper
    {
        public static OrderResponseDto ToResponse(this Order order) => new()
        {
            ...
            PaymentStrategyName = order.PaymentStrategy.HasValue ? ((PaymentStrategiesEnum)order.PaymentStrategy.Value).ToString() : null,
            OrderType = order.OrderType,
            OrderTypeName = order.OrderType.HasValue ? ((OrderTypeEnum)order.OrderType.Value).ToString() : null,
            Note = order.Note,
            ...
            NurseryOrders = order.NurseryOrders.Select(no => new NurseryOrderResponseDto
            {
                ...
                TotalAmount = no.SubTotalAmount ?? order.TotalAmount,
                DepositAmount = no.DepositAmount ?? order.DepositAmount,
                RemainingAmount = no.RemainingAmount ?? order.RemainingAmount,
                Status = no.Status,
